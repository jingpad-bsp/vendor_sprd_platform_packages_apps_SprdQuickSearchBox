...
import com.android.quicksearchbox.AppsCorpus;
...
public class SearchableItemsController implements PreferenceController, OnPreferenceChangeListener {
    ...
    public void handlePreference(Preference corporaPreferences) {
        ...
        getSettings().broadcastSettingsChanged(); // UNISOC: Modify for bug1163028
    }
    ...
    private void populateSourcePreference() {
        ...
        for (Corpus corpus : getCorpora().getAllCorpora()) {
            if (includeNonAllCorpora || corpus.includeInAll()) {
                ...
                /* UNISOC: Modify for bug 1074040 @{ */
                /*if (corpus instanceof AppsCorpus && Util.DEBUG) {
                    if (DBG)
                        Log.d(TAG, "Monkey test not Adding Appcorpus: " + corpus);
                } else {
                    if (pref != null) {
                        if (DBG)
                            Log.d(TAG, "Adding corpus: " + corpus);
                        mCorporaPreferences.addPreference(pref);
                    }
                }*/
                if (pref != null && !corpus.isWebCorpus()) {
    ...
    private Preference createCorpusPreference(Corpus corpus) {
        ...
        sourcePref.setDefaultValue(true); // UNISOC: Modify for bug1163028

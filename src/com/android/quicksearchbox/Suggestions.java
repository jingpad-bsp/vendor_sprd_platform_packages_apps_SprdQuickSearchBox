...
public class Suggestions {
    ...
    public Suggestions(String query, List<Corpus> expectedCorpora) {
        ...
        mLableFromSuggestions = new LinkedHashSet<String>();
    }
    ...
    public void addCorpusResults(List<CorpusResult> corpusResults) {
        ...
        initIndex((ArrayList)getCorpusResults());
        notifyDataSetChanged();
    }

    private volatile LinkedHashSet<String> mLableFromSuggestions;
    private volatile String[] mSections;
    private volatile int[] mCounts;

    private SearchSectionIndexer mSearchSectionIndexer;

    private void initIndex(final List<CorpusResult> CorpusResults) {
        String label = null;

        // fill mLable From Suggestions with all lables(Apps, Contacts, Music, Message...)
        for (int i = 0; i < CorpusResults.size(); i++) {
            label = (String)CorpusResults.get(i).getCorpus().getLabel();
            if (DBG) Log.d(TAG, "initIndex()->label = " + label);
            if (null != label) {
                mLableFromSuggestions.add(label);
            }// TODO else add other
        }
        // Section for indexer
        mSections = (String[]) mLableFromSuggestions.toArray(new String[mLableFromSuggestions.size()]);

        // label-position map(such as {[App, 0], [Contacts, 1],...}
        HashMap<String, Integer> map = new HashMap<String, Integer>();
        for (int i = 0; i < mSections.length; i++) {
            map.put(mSections[i], i);
            if (DBG) Log.d(TAG, "initIndex()->mSections[" + i + "] = " + mSections[i]);
        }

        // Count every sections
        mCounts = new int[mSections.length];
        int index = -1;
        for (int i = 0; i < CorpusResults.size(); i++) {
            final String corpusLabel = (String)CorpusResults.get(i).getCorpus().getLabel();
            final int count = CorpusResults.get(i).getCount();
            if (DBG) Log.d(TAG, "initIndex()->corpusLabel = " + corpusLabel);
            if (DBG) Log.d(TAG, "initIndex()->count = " + count);
            if (!TextUtils.isEmpty(corpusLabel)) {
                index = map.get(corpusLabel);
                if (index >= 0) {
                    mCounts[index] += count;
                }
            }
        ...
        if (DBG) {
            int  i = 0;
            for (String str : mLableFromSuggestions) {
                Log.d(TAG, "initIndex->label = " + str);
                Log.d(TAG, "initIndex->mSections[" + i + "] = " + mSections[i]);
                Log.d(TAG, "initIndex->mCounts[" + i + "] = " + mCounts[i]);
                i++;
            }
        }

        mSearchSectionIndexer = new SearchSectionIndexer(mSections, mCounts);
    }

    public SearchSectionIndexer getSearchSectionIndexer() {
        return mSearchSectionIndexer;
    }

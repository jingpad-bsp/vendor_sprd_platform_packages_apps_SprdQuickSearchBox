...
import static com.android.quicksearchbox.util.Util.ENGINES;
import static com.android.quicksearchbox.util.Util.MYENGINE;
...
//                    + "&q=" + URLEncoder.encode(query, "UTF-8");
    ...
    public String getSearchUrl(String source,String query) {
        ...
        String saveEngine = Settings.Global.getString(this.getContentResolver(), MYENGINE);

        if (ENGINES[4].equals(saveEngine)) {
            // bing
            baseUrl = this.getResources().getString(R.string.common_search_base_pattern, getSearchDomain(4));
            query = "/search?q=" + query;
        } else if (ENGINES[3].equals(saveEngine)) {
            // yahoo
            baseUrl = this.getResources().getString(R.string.common_search_base_pattern, getSearchDomain(3));
            // SPRD: modify this for Bug210932 at 06/09/2013
            query = "/s?p=" + query;
        } else if (ENGINES[2].equals(saveEngine)) {
            // 139
            baseUrl = this.getResources().getString(R.string.common_search_base_pattern, getSearchDomain(2));
            // SPRD: modify this for Bug210932 at 06/09/2013
            query = "/page/search?rs=monternet&q=" + query;
            return baseUrl + query;
        /* SPRD: Modify for bug 664809 {@ */
        /* SPRD: Modify for bug 620610 change default SearchEngine to google {@ */
        } else if (ENGINES[1].equals(saveEngine)) {
            // google
            baseUrl = this.getResources().getString(R.string.google_search_base_pattern, getSearchDomain(1),
                    GoogleSearch.getLanguage(Locale.getDefault()));
            query = "&q=" + query;
        } else if (ENGINES[0].equals(saveEngine)) {
            // baidu
            baseUrl = this.getResources().getString(R.string.common_search_base_pattern, getSearchDomain(0));
            query = "/s?wd=" + query;
        } else {
            // control by ro.operator
            if (this.getResources().getBoolean(R.bool.baidu_default_engine)) { // UNISOC: Modify for bug 1062120
                // baidu
                baseUrl = this.getResources().getString(R.string.common_search_base_pattern, getSearchDomain(0));
                query = "/s?wd=" + query;
            } else {

...
public class ShortcutRepositoryImplLog implements ShortcutRepository {
    ...
    private void runTransactionAsync(final SQLiteTransaction transaction) {
        mLogExecutor.execute(new Runnable() {
            public void run() {
                /* SPRD：530150 avoid crash when storage is full @{ */
                try {
...
// -------------------------- end ShortcutRepository --------------------------
     ...
     */
    /*Add 201305231 Spreadst of 172701 cant create the cursor start */
    synchronized Map<String,Integer> getCorpusScores(int minClicks) {
    /*Add 201305231 Spreadst of 172701 cant create the cursor end */
        /* SPRD：530150 avoid crash when storage is full @{ */
        SQLiteDatabase db = null;
        try {
            db = mOpenHelper.getReadableDatabase();
